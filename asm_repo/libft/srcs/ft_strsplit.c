...
/*   Updated: 2017/02/22 12:28:30 by jjourdai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char		**ft_strsplit(char const *s, char c)
{
	...
	if (!s)
		return (NULL);
	if ((tab = ft_memalloc(sizeof(char*) * ft_nb_word(s, c, &size) + 1)) == 0)
		return (NULL);
	i = 0;
	j = 0;
	while (size--)
	{
		while (s[i] && s[i] == c)
			i++;
		k = 0;
		tab[j] = malloc(sizeof(char) * ft_caract_number((char*)s, i, c) + 1);
		while (s[i] && s[i] != c)
			tab[j][k++] = s[i++];
		tab[j++][k] = '\0';
		i++;
